...
<div class="grid grid-cols-2 lg:grid-cols-4 divide-x divide-y divide-fyra-gray-800">
	{#each riscvPlans as plan, i}
		<button
			...
			class="col-span-1 w-full p-4 duration-200 text-left {selectedPlanName === plan.name ? 'bg-fyra-gray-800' : 'bg-fyra-gray-900 hover:bg-fyra-gray-800'} {i === riscvPlans.length - 1 ? 'border-b border-fyra-gray-800' : ''}"
		>
